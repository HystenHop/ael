...
Versão 0.1.1 · catálogo, API REST e integração com Power BI.

Projeto com objetivo de **prática, estudo e revisão dos processos envolvidos** em
desenvolvimento back-end, banco de dados relacional, segurança, testes e análise de dados.
...
- exportação do SQLite para Power BI, com consultas Power Query, medidas DAX e tema;
- API REST de materiais com sessão, CSRF, paginação e respostas JSON;
...
| `ael/api.py` | API JSON de materiais com autenticação e paginação |
...
- [Power BI: importação, medidas e painel](powerbi/README.md)
- [API: contrato e exemplo PowerShell](docs/API.md)
...
Para uma demonstração descartável, selecione um banco fora do pacote com
`$env:AEL_DATABASE = Join-Path $PWD 'demo.sqlite3'` antes dos comandos Flask.
Sem essa variável, o padrão continua sendo `instance/ael.sqlite3`.
...
A suíte automatizada verifica a API REST e os fluxos web: autenticação, permissões, CSRF, validações,
...
Desenvolvido com apoio de IA; decisões, testes e limitações estão documentados.
